...
uint8_t at24writebuff[10] = {0x00};
#define SIZE sizeof(TEST_BUFFER)
/************************************************
   每个从机用10个字节保存 
   1. 是否已经写入 0xa5  写入 否则 未写入 ,index =0
   2. 从机号                              ,index =1
   3-4  长                                ,index =2,3
   5-6  宽                                ,index =4,5
   7-8  高                                ,index =6,7
...
//    for(i=0;i<AT24CXX_MAX_MEM_ADDRESS;i++)
//        TEST_BUFFER[i] = i;
    dev = at24cxx_init("i2c1", 0x50);
    if (at24cxx_check(dev) == 1)
    {
        rt_kprintf("at24cxx check faild!!! \n");
    }
    else
    {
        rt_kprintf("at24cxx check success !!!\n");
    }

    
     if (RT_EOK ==at24cxx_read(dev, 0, readbuffer, 100))
     {
         for(i=0;i<100;i=i+10)
         {
               if(readbuffer[i] ==0xa5) //已经保存了该从机的数据,读取到全局从机变量中
               {
               g_Slave[i/10].slaveaddr      = readbuffer[i+1];
               g_Slave[i/10].slavelegth     = readbuffer[i+2]*256+readbuffer[i+3];
               g_Slave[i/10].slavewidth     = readbuffer[i+4]*256+readbuffer[i+5];
               g_Slave[i/10].slaveheight    = readbuffer[i+6]*256+readbuffer[i+7];
               g_Slave[i/10].slaveonoff     = readbuffer[8];
               g_Slave[i/10].slavemode      = readbuffer[9];
               }
         }
     }
     else
...
// 根据从机号 保存模式 开关
// 保存的起始地址  (从机号(1开始)-1 )*10
// 
void bsp_at24cxx_savedata(uint8_t address,uint8_t *data, uint8_t size)
{
    if(RT_EOK == at24cxx_write(dev, address, data, size))
    {
